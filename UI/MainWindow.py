...
class MainWindow(QWidget):
    ...
    def video_capture_thread(self):
        while True:
            ...
            if ret:
                ...
                try:
                    response = \
                        model.generate_content(["Describe what is happening in front of the camera.", image_pil]).parts[
                            0].text
    ...
    async def capture_question(self):
        ...
        if ret:
            image_pil = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            try:
                history_texts = [f"At {item['timestamp']}, you saw: {item['description']}" for item in
                                 self.interaction_memory]
                full_context = "\n".join(history_texts)
                response = await self.loop.run_in_executor(None, lambda:
                ...
                self.answer_edit.append(f"<b>AI Response:</b> {response}")
    ...
    def update_webcam_response(self, response):
        ...
        self.webcam_response_edit.append(f"<b>AI Response:</b> {response}")
